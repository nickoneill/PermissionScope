...
@objc public class PermissionScope: UIViewController, UIGestureRecognizerDelegate {
	...
	*/
	public init(backgroundTapCancels: Bool) {
		...
		self.trySelector(named: "statusMotion") //Added to check motion status on load
	}

	func trySelector(named: String){ // Convenience method for using methods hidden in Extensions
		let sel = Selector(named)
	...
	*/
	@objc public func addPermission(_ permission: Permission, message: String) {
		...
		if permission.type == .bluetooth && askedBluetooth {
			trySelector(named: "triggerBluetoothStatusUpdate")
		} else if permission.type == .motion && askedMotion {
			trySelector(named: "triggerMotionStatusUpdate")
		}
	}
	...
	*/
	func statusForPermission(_ type: PermissionType, completion: statusRequestClosure) {
		let status = self.statusForType(type)
		completion(status)
	}

	func statusForType(_ type: PermissionType) -> PermissionStatus {
		let name = "status\(type.description)"
		let sel  = Selector(name)

		if self.responds(to: sel) {
			let val = self.value(forKey: name) as! NSNumber
			return PermissionStatus(rawValue: val.intValue)!
		}
		return .unknown
	}
